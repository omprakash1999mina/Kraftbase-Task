...
const forgotPasswordController = {
    ...
    async forgot(req, res, next) {
        ...
        try {
            ...
            const exist = await Agent.exists({ email: email });
            if (!exist) {
                discord.SendErrorMessageToDiscord(email, "Forgot password", "Agent not exist in our database !!");
                return next(CustomErrorHandler.unAuthorized())
            }
            // verifying the otp with redis
            let redisOTP;
            ...
            let document = await Agent.findOneAndUpdate({ email: email }, {
